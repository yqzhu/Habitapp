...
const CANONICAL_ATTRIBUTES = ['Physique', 'Charisma', 'Wisdom', 'Sociability', 'Farming', 'Wealth', 'Survival'];
const CANONICAL_TIERS = ['Paper', 'Rock', 'Bronze', 'Silver', 'Gold'];

function normalizeCadenceRule(input: string): string {
  const raw = input.trim();
  const lower = raw.toLowerCase();

  if (['daily', 'every day', 'everyday'].includes(lower)) {
    return 'daily';
  }

  if (['every other day', 'alternate day', 'every 2 days', 'every two days'].includes(lower)) {
  ...
  const tokens = lower.replace(/&/g, ',').replace(/and/g, ',').split(/[,\s]+/).filter(Boolean);
...
app.post('/api/task-templates', async (req, res) => {
  ...
  if (!CANONICAL_ATTRIBUTES.includes(attribute) || !CANONICAL_TIERS.includes(baseTier)) {
    return res.status(400).json({ error: 'attribute or baseTier is invalid' });
  }

  const template = await prisma.taskTemplate.create({
    data: { title, cadenceRule: normalizeCadenceRule(cadenceRule), attribute, baseTier, isActive: true },
  });
...
app.patch('/api/task-templates/:id', async (req, res) => {
  ...
  if (attribute && !CANONICAL_ATTRIBUTES.includes(attribute)) {
    return res.status(400).json({ error: 'attribute is invalid' });
  }
  if (baseTier && !CANONICAL_TIERS.includes(baseTier)) {
    return res.status(400).json({ error: 'baseTier is invalid' });
  }

  const template = await prisma.taskTemplate.update({
    where: { id },
    data: { title, cadenceRule: cadenceRule ? normalizeCadenceRule(cadenceRule) : cadenceRule, attribute, baseTier, isActive },
  });
